...
def random_image_and_indices_mutation(images, uv_pixel_positions):
    ...
    #TODO disable Flip augmentation
...
def flip_vertical(images, uv_pixel_positions):
    ...
    mutated_v_pixel_positions = (image_height-1) - v_pixel_positions
...
def flip_horizontal(images, uv_pixel_positions):
    ...
    mutated_u_pixel_positions = (image_width-1) - u_pixel_positions
...
def apply_randomization_full_image(image):
    # Example randomization parameters
    brightness_factor = np.random.uniform(0.5, 1.1)
    contrast_factor = np.random.uniform(0.5, 1.2)
    noise = np.random.normal(0, 0.3, image.shape).astype(np.uint8)

    # Apply randomization
    randomized_image = cv2.convertScaleAbs(image, alpha=contrast_factor, beta=brightness_factor)
    randomized_image = cv2.add(randomized_image, noise)

    # Randomly select a blur kernel size (odd numbers work best)
    blur_kernel_size = np.random.choice([3, 5, 7])
    # Apply blur
    randomized_image = cv2.GaussianBlur(randomized_image, (blur_kernel_size, blur_kernel_size), 0)
