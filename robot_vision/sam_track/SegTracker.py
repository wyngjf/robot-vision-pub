...
from robot_vision.sam_track.aot_tracker import get_aot
import numpy as np
from robot_vision.sam_track.tool.segmentor import Segmentor
# from robot_vision.sam_track.tool.detector import Detector
from robot_vision.sam_track.tool.transfer_tools import draw_outline, draw_points
import cv2
from robot_vision.sam_track.seg_track_anything import draw_mask
...
class SegTracker():
    ...
    def reset(self):
        console.log("resetting grounded sam track")
    ...
    def add_masks(self, frame: np.ndarray, masks: Union[List[np.ndarray], np.ndarray]):
        bc_id = self.curr_idx
        bc_mask = self.origin_merged_mask
        for i in range(len(masks)):
            mask = masks[i]
            refined_merged_mask = self.add_mask(mask)
            self.update_origin_merged_mask(refined_merged_mask)
            self.curr_idx += 1

        # reset origin_mask
        self.reset_origin_merged_mask(bc_mask, bc_id)
        self.add_reference(frame, refined_merged_mask)
